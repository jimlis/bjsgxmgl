package com.zj.platform.common.util.conversion;
...
public class CustomDateTimeEditor extends PropertyEditorSupport {
	...
	public void setAsText(String text) throws IllegalArgumentException {
		if (this.allowEmpty && !StringUtils.hasText(text)) {
			...
			try {
				if(text.indexOf("CST") > 0){
					SimpleDateFormat cstFormat = new SimpleDateFormat("EEE MMM dd HH:mm:ss 'CST' yyyy", Locale.US);
					setValue(cstFormat.parse(text));
				}
				else{
					if(text.length() == 10){
						text = text + " 00:00:00";
