...
public class ApiXmGckyzfqkController extends ApiBaseController {
    ...
    public Result<List<XmGckyzfqkDO>> getXmGckyzfqkListByXmidAndDwlx(Long xmid,Integer dwlx) {
        try {
        	...
        	QueryWrapper<XmGckyzfqkDO> queryWrapper=new QueryWrapper<XmGckyzfqkDO>(xmGckyzfqkDO).select("id","dtmgxrq","intdwmcid","intbcsqqs","intdwlx","chrspzt","(select chrdwmc from bj_xm_dwmd where id=intdwmcid) as chrdwmc").orderByAsc("intdwmcid","intbcsqqs");
